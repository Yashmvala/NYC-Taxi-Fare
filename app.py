...
with st.sidebar:
    ...
    st.write("- Inputs → Output: distance, time/day, passengers, pickup + dropoff zones, rain → **total fare ($)**")
    if model_available:
        st.success("Model loaded")
    else:
        st.warning("No model file found — will use simple formula")
...
if st.button("🔮 Predict fare"):
    if model_available:
        try:
            ...
            st.warning("Model failed to load — using simple formula.")
    ...
    st.metric("Suggested range", f"${low:.2f} — ${high:.2f}")
...
# ---------------------------
st.write("")
st.caption("Built for a class project — simple features, small model, clear demo.")